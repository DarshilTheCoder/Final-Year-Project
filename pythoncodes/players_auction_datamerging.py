...
STATS_CSV   = r"D:\DataEngineering\Final Year Project\processed_data\players_data\all_players_combined.csv"
...
merged.to_csv(Path(OUTPUT_DIR) / "auction_with_prior_stats.csv", index=False)
...
print("2. duplicate (id,year) :", int(merged.duplicated(["player_id", "year"], keep=False).sum()),
      "(want 0)")
