"""
Check the auction<->stats merge is correct or not and matching id summary of in unsold data.

"""

import pandas as pd
from pathlib import Path


AUCTION_CSV = r"D:\DataEngineering\Final Year Project\processed_data\auction_all_with_base_price.csv"
MERGED_CSV  = r"D:\DataEngineering\Final Year Project\processed_data\final_4_with_status_auction_with_prior_stats.csv"


auc = pd.read_csv(AUCTION_CSV)
m = pd.read_csv(MERGED_CSV)
has = m["has_prior_season"] == True

print("=" * 60)
print("STRUCTURAL CHECKS (automatic - these should all say OK)")
print("=" * 60)


print(f"1. rows: auction={len(auc)}  merged={len(m)}  ->",
      "OK" if len(m) == len(auc) else "PROBLEM: rows changed (stats had duplicate keys)")

d = int(m.duplicated(["player_id", "year"], keep=False).sum())
print(f"2. duplicate (player_id, year) rows: {d}  ->",
      "OK" if d == 0 else "PROBLEM: same player appears twice in a year")


lag_ok = (m.loc[has, "prior_season"] == m.loc[has, "year"] - 1).all()
print(f"3. every matched row has prior_season == year-1  ->",
      "OK (no leakage)" if lag_ok else "PROBLEM: wrong season attached")


print(f"4. rows with prior-season stats: {has.sum()}/{len(m)} ({100*has.mean():.0f}%)"
      f"   [the rest are debutants/uncapped - expected]")

print("\n" + "=" * 60)
print("MANUAL SPOT-CHECK (eyeball these, then verify on ESPNcricinfo)")
print("=" * 60)
print("For each player: 'lag' must be 1, and the runs/wickets should match")
print("that player's REAL IPL stats for the 'prior_season' shown.\n")


famous = ["Kohli", "Rohit", "Dhoni", "Rayudu", "Jadeja",
          "Bumrah", "Warner", "Russell", "Rashid", "Buttler"]
m["lag"] = m["year"] - m["prior_season"]
cols = ["player_name", "year", "prior_season", "lag",
        "matches", "runs", "wickets", "team_stats"]
rows = [m[has & m["player_name"].str.contains(n, case=False, na=False)][cols].head(1)
        for n in famous]
rows = [r for r in rows if len(r)]
if rows:
    print(pd.concat(rows, ignore_index=True).to_string(index=False))




UNSOLD_WITH_IDS = [r"/Users/umerkarachiwala/Desktop/Darshil'sFinalYearProject/Final-Year-Project/processed_data/unsold_data/unsold_with_base_price0_with_ids.csv",
                r"/Users/umerkarachiwala/Desktop/Darshil'sFinalYearProject/Final-Year-Project/processed_data/unsold_data/unsold_with_base_price1_with_ids.csv",
                r"/Users/umerkarachiwala/Desktop/Darshil'sFinalYearProject/Final-Year-Project/processed_data/unsold_data/unsold_with_base_price_2_with_ids.csv"]

src = pd.concat([pd.read_csv(f)["id_source"] for f in UNSOLD_WITH_IDS],
                 ignore_index=True).astype(str)
total = len(src)

group = {
    "already had id":           "Already present on source page",
    "name match":               "Exact name match (automatic)",
    "initial match":            "Initial + surname match (automatic)",
    "ambiguous name":           "Ambiguous - multiple candidates (manual review)",
    "ambiguous initial":        "Ambiguous - multiple candidates (manual review)",
    "not found - add manually": "No automatic match (manual entry)",
}
order = [
    "Already present on source page",
    "Exact name match (automatic)",
    "Initial + surname match (automatic)",
    "Ambiguous - multiple candidates (manual review)",
    "No automatic match (manual entry)",
]
tab = (src.map(group).value_counts()
             .reindex(order, fill_value=0)
             .rename_axis("match_status").reset_index(name="records"))
tab["percent"] = (tab["records"] / total * 100).round(1)

print("\n" + "=" * 60)
print("PLAYER-ID MATCH STATUS (unsold / base-price records)")
print("=" * 60)
print(tab.to_string(index=False))
print(f"{'TOTAL':50s} {total:5d}  100.0")